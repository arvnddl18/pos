...
kdsRoutes.get("/queue", requireAuth(), async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT tk.id, tk.ticket_no, tk.status, COALESCE(tk.kds_state, 'preparing') AS kds_state, tk.ticket_type, tk.parked_label, tk.updated_at,
